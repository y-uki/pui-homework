...
function setCheckoutLineWidth() {
    // Get all cartItem elements
    const cartItems = document.querySelectorAll('.cartItem');
    
    // Initialize maxWidth variable to store the maximum width
    let maxWidth = 0;

    // Loop through each cartItem to find the maximum width
    cartItems.forEach(item => {
        // Get the width of the current cartItem
        const itemWidth = item.offsetWidth;
        
        // Update maxWidth if the current cartItem width is greater
        if (itemWidth > maxWidth) {
            maxWidth = itemWidth;
        }
    });

    // Set the width of checkoutLine to the maximum width found
    const checkoutLine = document.getElementById('checkoutLine');
    checkoutLine.style.width = maxWidth + 'px';
}

// Call the function to set the width of checkoutLine
setCheckoutLineWidth();